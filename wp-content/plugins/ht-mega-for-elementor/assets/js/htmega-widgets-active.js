(function ($) {
  ...
  var WidgetTestimonialCarouselHandler = function ($scope, $) {
    ...
    if (carousel_elem.length > 0) {
      ...
      var testimonial_style_ck =
        parseInt(settings["testimonial_style_ck"]) || 1;
      ...
      if (testimonial_style_ck == 5 || testimonial_style_ck == 11) {
        var carousel_elem_for = $scope
          .find(".htmega-testimonial-activation .htmega-testimonial-for")
          .eq(0);
        if (carousel_elem_for.length > 0) {
          carousel_elem_for[0].style.display = "block";
        }
        var carousel_elem_nav = $scope
          .find(".htmega-testimonial-activation .htmega-testimonal-nav")
          .eq(0);
        ...
        carousel_elem_for.slick({
          ...
          prevArrow:
            '<button aria-label="' +
            areaBtnPrev +
            '" type="button" class="slick-prev">' +
            arrow_prev_txt +
            "</button>",
          nextArrow:
            '<button aria-label="' +
            areaBtnNext +
            '" type="button" class="slick-next">' +
            arrow_next_txt +
            "</button>",
          dots: dots,
          fade: true,
  ...
  var WidgetHtmegaCarouselHandler = function ($scope, $) {
    ...
    if (carousel_elem.length > 0) {
      ...
      var pause_on_hover = settings["pause_on_hover"];
      var center_mode = settings["center_mode"];
      var verticalMode = settings["vertical_mode"]
        ? settings["vertical_mode"]
        : false;
      var slloop = settings["slloop"];
      var center_padding = settings["center_padding"]
        ? parseInt(settings["center_padding"])
        : 0;
      ...
      if (carousel_style_ck == 4) {
        carousel_elem.slick({
          arrows: arrows,
          prevArrow:
            '<button class="htmega-carosul-prev">' +
            arrow_prev_txt +
            "</button>",
          nextArrow:
            '<button class="htmega-carosul-next">' +
            arrow_next_txt +
            "</button>",
          dots: dots,
          customPaging: function (slick, index) {
            var data_title = slick.$slides
              .eq(index)
              .find(".htmega-data-title")
              .data("title");
            var data_thumbnail = slick.$slides
              .eq(index)
              .find(".htmega-data-title")
              .data("thumbnail");
            ...
            if (data_thumbnail) {
              return (
                '<img src="' + data_thumbnail + '" alt="' + data_title + '">'
              );
            } else {
        ...
        carousel_elem.slick({
          ...
          prevArrow:
            '<button aria-label="' +
            areaBtnPrev +
            '" class="htmega-carosul-prev">' +
            arrow_prev_txt +
            "</button>",
          nextArrow:
            '<button aria-label="' +
            areaBtnNext +
            '" class="htmega-carosul-next">' +
            arrow_next_txt +
            "</button>",
          dots: dots,
          infinite: true,
      ...
      //equal height column
      if (
        settings["equal_height_column"] &&
        settings["sectionid"] &&
        settings["equal_height_column_class"]
      ) {
        var maxHeight = -1;
        $(
          "." +
  ...
  var WidgetHtmegaThumbnailsgalleryHandler = function ($scope, $) {
    var thumbnailscarousel_elem_for = $scope
      .find(".htmega-thumbgallery-for")
      .eq(0);
    var thumbnailscarousel_elem_nav = $scope
      .find(".htmega-thumbgallery-nav")
      .eq(0);
    ...
    if (thumbnailscarousel_elem_for.length > 0) {
      ...
      var navautoplay_speed =
        parseInt(navsettings["navautoplay_speed"]) || 3000;
      var navanimation_speed =
        parseInt(navsettings["navanimation_speed"]) || 300;
      ...
      var navtablet_display_columns =
        parseInt(navsettings["navtablet_display_columns"]) || 1;
      var navtablet_scroll_columns =
        parseInt(navsettings["navtablet_scroll_columns"]) || 1;
      var navmobile_width = parseInt(navsettings["navmobile_width"]) || 480;
      var navmobile_display_columns =
        parseInt(navsettings["navmobile_display_columns"]) || 1;
      var navmobile_scroll_columns =
        parseInt(navsettings["navmobile_scroll_columns"]) || 1;
  ...
  function countDownFlip($this, $time, $countdownoptions) {
    var labels = ["Days", "Hours", "Mins", "Secs"],
      template = function (label, curr, next) {
        if (
          $countdownoptions["htmegaday"] == "yes" &&
          label.toLowerCase() == "days"
        ) {
          return `<div class="htmega-time htmega-days">
                            ...
                            <span class="htmega-label">${
                              $countdownoptions["lavelhide"] == "yes"
                                ? ""
                                : $("<p>")
                                    .text($countdownoptions["htmegadaytxt"])
                                    .html()
                            }</span>
                        </div>`;
        } else if (
          $countdownoptions["htmegahours"] == "yes" &&
          ...
          return `<div class="htmega-time htmega-hours">
                            ...
                            <span class="htmega-label">${
                              $countdownoptions["lavelhide"] == "yes"
                                ? ""
                                : $("<p>")
                                    .text($countdownoptions["htmegahourtxt"])
                                    .html()
                            }</span>
                        </div>`;
        } else if (
          $countdownoptions["htmegaminiute"] == "yes" &&
          label.toLowerCase() == "mins"
        ) {
          return `<div class="htmega-time htmega-mins">
                            ...
                            <span class="htmega-label">${
                              $countdownoptions["lavelhide"] == "yes"
                                ? ""
                                : $("<p>")
                                    .text($countdownoptions["htmegaminutestxt"])
                                    .html()
                            }</span>
                        </div>`;
        } else if (
          $countdownoptions["htmegasecond"] == "yes" &&
          label.toLowerCase() == "secs"
        ) {
          return `<div class="htmega-time htmega-secs">
                            ...
                            <span class="htmega-label">${
                              $countdownoptions["lavelhide"] == "yes"
                                ? ""
                                : $("<p>")
                                    .text($countdownoptions["htmegasecondstxt"])
                                    .html()
                            }</span>
                        </div>`;
        }
        ...
        if (countdownoptions["style"] === "flip") {
          ...
          $this.countdown(countdownoptions.htmegadate, function (event) {
            ...
            if (countdownoptions.lavelhide == "yes") {
              daysTime =
                '<span class="ht-count days"><span class="count-inner"><span class="time-count">%-D</span> </span></span>';
              hours =
                '<span class="ht-count hour"><span class="count-inner"><span class="time-count">%-H</span> </span></span>';
              minutes =
                '<span class="ht-count minutes"><span class="count-inner"><span class="time-count">%M</span> </span></span>';
              second =
                '<span class="ht-count second"><span class="count-inner"><span class="time-count">%S</span> </span></span>';
            } else {
              daysTime =
                '<span class="ht-count days"><span class="count-inner"><span class="time-count">%-D</span>' +
                $("<p>").text(countdownoptions.htmegadaytxt).html() +
                "</span></span>";
              hours =
                '<span class="ht-count hour"><span class="count-inner"><span class="time-count">%-H</span> ' +
                $("<p>").text(countdownoptions.htmegahourtxt).html() +
                "</span></span>";
              minutes =
                '<span class="ht-count minutes"><span class="count-inner"><span class="time-count">%M</span>' +
                $("<p>").text(countdownoptions.htmegaminutestxt).html() +
                "</span></span>";
              second =
                '<span class="ht-count second"><span class="count-inner"><span class="time-count">%S</span>' +
                $("<p>").text(countdownoptions.htmegasecondstxt).html() +
                "</span></span>";
            }
      ...
      $(notify_opt.notify_btn_class).on("click", function () {
        $.notify(
          {},
          {
            ...
            z_index: 99999,
            template:
              '<div class="htmega-alert-wrap-' +
              notify_opt.wrapid +
              " " +
              notify_opt.width +
              ' alert alert-{0}">' +
              '<span data-notify="dismiss" class="htmega-close"><i class="fas fa-times"></i></span>' +
              notify_opt.icon +
              '<span class="notify-message-content">' +
              DOMPurify.sanitize(notify_opt.notifymessage) +
              "</span></div>",
          }
        );
      });
    ...
    if (true == swiper_opt.slide_custom_menu) {
      $('a[href^="#htmega-scroll-slide"]').on("click", function (e) {
        ...
        var slideIndex = parseInt(
          fullIndex.replace("#htmega-scroll-slide-", ""),
          0
        );
    ...
    if (postgridtab_elem.length > 0) {
      $(".post-gridthumb-" + postgridtab_opt.wrapid + "").each(function () {
        $(this).on("click", function (e) {
          e.preventDefault();
          $(this)
            .next(".post-gridcontent-" + postgridtab_opt.wrapid + "")
            .slideToggle()
            .siblings(".post-gridcontent-" + postgridtab_opt.wrapid + "")
            .slideUp();
          $(this)
            .next(".post-gridcontent-" + postgridtab_opt.wrapid + "")
            .toggleClass("is-visible")
            .siblings(".post-gridcontent-")
            .removeClass("is-visible");
        });
      });

      $(".post-gridclose-" + postgridtab_opt.wrapid + "").on(
        "click",
        function () {
          $(this)
            .parent(".post-gridcontent-" + postgridtab_opt.wrapid + "")
            .slideUp();
        }
      );
    }
  ...
  var HTMegaAudioPlayer = function ($scope, $) {
    ...
    if (container_elem.length > 0) {
      ...
      activeUniqClass.mediaelementplayer({
        ...
        success: function (mediaElement, originalNode, instance) {
          ...
          mediaElement.addEventListener("progress", function () {
            ...
            const durationContainer = $(mediaElement)
              .siblings(".mejs__time-total")
              .find(".mejs__duration");
      ...
      if (settings?.playerIcons) {
        let playPauseButton = container_elem.find(
        ...
        playPauseButton.html(
          ...
          volumeButtion.html(
            `<i aria-hidden="true" class="htmega-audio-unmute ${settings.playerIcons.unmute}"></i><i aria-hidden="true" class="htmega-audio-mute ${settings.playerIcons.mute}"></i>`
          );
      }
    }
  };
  ...
  $(window).on("elementor/frontend/init", function () {
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-tab-addons.default",
      WidgetTabControll
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-google-map-addons.default",
      WidgetGoogleMapHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-accordion-addons.default",
      WidgetAccordionsMapHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-progressbar-addons.default",
      WidgetPieChartMapHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-countdown-addons.default",
      WidgetCountdownMapHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-team-member-addons.default",
      WidgetTeamMemberPlusHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-testimonial-addons.default",
      WidgetTestimonialCarouselHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-newtsicker-addons.default",
      WidgetNewsTrickerHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-magnific-popup-addons.default",
      WidgetLightboxHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-videoplayer-addons.default",
      WidgetVideoPlayerHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-instagram-addons.default",
      WidgetHtmegaCarouselHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-tooltip-addons.default",
      WidgetTooltipHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-popover-addons.default",
      WidgetPopoversHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-notify-addons.default",
      WidgetNotifyHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-counter-addons.default",
      WidgetCounterHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-postslider-addons.default",
      WidgetHtmegaCarouselHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-thumbgallery-addons.default",
      WidgetHtmegaThumbnailsgalleryHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-imagemasonryd-addons.default",
      WidgetImageMasonaryHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-scrollnavigation-addons.default",
      WidgetNavigationScrollHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-twitterfeed-addons.default",
      WidgetHtmegaCarouselHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-carousel-addons.default",
      WidgetHtmegaCarouselHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-postcarousel-addons.default",
      WidgetHtmegaCarouselHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-postgridtab-addons.default",
      WidgetPostGridHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-imagecomparison-addons.default",
      WidgetImageComparisonHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-panelslider-addons.default",
      WidgetHtmegaCarouselHandler
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-switcher-addons.default",
      WidgetSwithcerControll
    );
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/htmega-audio-player-addons.default",
      HTMegaAudioPlayer
    );
  });
})(jQuery);
